...
class CsvDB {
  constructor(file, fields = null) {
    this.file = file
    this.fields = fields
    this.data = this.transform(fs.readFileSync(file, 'utf-8'))
  }
  ...
  search(text) {
    ...
    if (text) {
      result = this.data.filter(row => {
        if (row['stop_code'] && row['stop_name']) {
          return (
            row['stop_code'].includes(text) || row['stop_name'].toLowerCase().includes(text)
          )
        } else {
          return false
        }
